export enum ProfileCode {
    ADMIN = 'ADMIN',
    EMPLOYER = 'EMPLOYER',
    RECRUITER = 'RECRUITER',
    CANDIDATE = 'CANDIDATE',
}

export enum RoleCode {
    PLATFORM_ADMIN = 'PLATFORM_ADMIN',
    ADMINISTRATOR = 'ADMINISTRATOR',
    HIRING_TEAM = 'HIRING_TEAM',
    INTERNAL_RECRUITER = 'INTERNAL_RECRUITER',
    NO_ACCESS = 'NO_ACCESS',
    RECRUITER = 'RECRUITER',
    CANDIDATE = 'CANDIDATE',
}

export enum RightCode {
    VIEW_JOBS_LIST = 'VIEW_JOBS_LIST',
    VIEW_JOB = 'VIEW_JOB',
    CREATE_JOB = 'CREATE_JOB',
    EDIT_JOB = 'EDIT_JOB',
    DELETE_JOB = 'DELETE_JOB',
    
    VIEW_SETTINGS = 'VIEW_SETTINGS',
    EDIT_SETTINGS = 'EDIT_SETTINGS',

    VIEW_PROFILE = 'VIEW_PROFILE',
    EDIT_PROFILE = 'EDIT_PROFILE',

    VIEW_COMPANY = 'VIEW_COMPANY',
    EDIT_COMPANY = 'EDIT_COMPANY',

    VIEW_TEAM = 'VIEW_TEAM',
    INVITE_MEMBER = 'INVITE_MEMBER',
    EDIT_MEMBER = 'EDIT_MEMBER',
    DELETE_MEMBER = 'DELETE_MEMBER',

    VIEW_RECRUITERS_LIST = 'VIEW_RECRUITERS_LIST',
    VIEW_RECRUITER = 'VIEW_RECRUITER',
    APPROVE_RECRUITER = 'APPROVE_RECRUITER',
    REJECT_RECRUITER = 'REJECT_RECRUITER',
    AUTOAPPROVE_RECRUITER = 'AUTOAPPROVE_RECRUITER',

    VIEW_JOB_KANBAN = 'VIEW_JOB_KANBAN',
    MOVE_CANDIDATE_TO_STAGE = 'MOVE_CANDIDATE_TO_STAGE',
    ADD_KANBAN_STAGE = 'ADD_KANBAN_STAGE',
    EDIT_KANBAN_STAGE_NAME = 'EDIT_KANBAN_STAGE_NAME',
    // EDIT_KANBAN_STAGE_TYPE = 'EDIT_KANBAN_STAGE_TYPE',
    DELETE_KANBAN_STAGE = 'DELETE_KANBAN_STAGE',
    HIRE_CANDIDATE = 'HIRE_CANDIDATE',
    REJECT_CANDIDATE = 'REJECT_CANDIDATE',
    ADD_KANBAN_STAGE_TRIGGER = 'ADD_KANBAN_STAGE_TRIGGER',
    EDIT_KANBAN_STAGE_ORDER = 'EDIT_KANBAN_STAGE_ORDER',

    VIEW_REVIEWS = 'VIEW_REVIEWS',
    CREATE_REVIEW = 'CREATE_REVIEW',

    VIEW_MESSAGES = 'VIEW_MESSAGES',
    SEND_MESSAGE = 'SEND_MESSAGE',

    VIEW_NOTES = 'VIEW_NOTES',
    ADD_NOTE = 'ADD_NOTE',

    VIEW_TODOS = 'VIEW_TODOS',
    ADD_TODO = 'ADD_TODO',
    EDIT_TODO = 'EDIT_TODO',
    DELETE_TODO = 'DELETE_TODO',

    // VIEW_INTERVIEWS = 'VIEW_INTERVIEWS',
    // SCHEDULE_INTERVIEW = 'SCHEDULE_INTERVIEW',
    // EDIT_INTERVIEW = 'EDIT_INTERVIEW',
    // CANCEL_INTERVIEW = 'CANCEL_INTERVIEW',

    VIEW_EVENTS_LIST = 'VIEW_EVENTS_LIST',
    VIEW_EVENT = 'VIEW_EVENT',
    CREATE_EVENT = 'CREATE_EVENT',
    RESCHEDULE_OWN_EVENT = 'RESCHEDULE_OWN_EVENT',
    REFUSE_OWN_EVENT = 'REFUSE_OWN_EVENT',
    CONFIRM_OWN_EVENT = 'CONFIRM_OWN_EVENT',
    RESCHEDULE_TARGET_EVENT = 'RESCHEDULE_TARGET_EVENT',
    REFUSE_TARGET_EVENT = 'REFUSE_TARGET_EVENT',
    CONFIRM_TARGET_EVENT = 'CONFIRM_TARGET_EVENT',
    RESCHEDULE_OTHER_EVENT = 'RESCHEDULE_OTHER_EVENT',
    REFUSE_OTHER_EVENT = 'REFUSE_OTHER_EVENT',
    CONFIRM_OTHER_EVENT = 'CONFIRM_OTHER_EVENT',
    DELETE_OWN_EVENT = 'DELETE_OWN_EVENT',
    DELETE_TARGET_EVENT = 'DELETE_TARGET_EVENT',
    DELETE_OTHER_EVENT = 'DELETE_OTHER_EVENT',
    EDIT_OWN_EVENT = 'EDIT_OWN_EVENT',
    EDIT_TARGET_EVENT = 'EDIT_TARGET_EVENT',
    EDIT_OTHER_EVENT = 'EDIT_OTHER_EVENT',
}

export enum JobTypeCode {
    FULL_TIME = 'FULL_TIME',
    PART_TIME = 'PART_TIME',
    CONTRACT = 'CONTRACT',
}

export enum PresenceStatusCode {
    ONSITE = 'ONSITE',
    REMOTE = 'REMOTE',
    HYBRID = 'HYBRID',
}

export enum UrgencyCode {
    REGULAR = 'REGULAR',
    URGENT = 'URGENT',
}

export enum TagTypeCode {
    TAG = 'TAG',
    SKILL = 'SKILL',
}

export enum SkillTypeCode {
    TECHNICAL = 'TECHNICAL',
    OTHER = 'OTHER',
    BONUS = 'BONUS',
}

export enum JobApplicationStatusCode {
    AUTOAPPROVED = 'AUTOAPPROVED',
    APPROVED = 'APPROVED',
    PENDING = 'PENDING',
    REJECTED = 'REJECTED',
    WITHDRAWN = 'WITHDRAWN',
}

export enum StageTypeName {
    SCREENING = 'Screening',
    INTERVIEW = 'Interview',
    OFFER = 'Offer',
}

export enum StageCategoryName {
    MANDATORY = 'Mandatory',
    DEFAULT = 'Default',
    CUSTOM = 'Custom',
}

export enum StageOwnerTypeName {
    EMPLOYER = 'Employer',
    RECRUITER = 'Recruiter',
}

export enum CalendarEventStatusName {
    PENDING = 'Pending',
    CONFIRMED = 'Confirmed',
    REFUSED = 'Refused',
}

export enum CalendarEventTypeName {
    INTERVIEW = 'Interview',
    MEETING = 'Meeting',
}